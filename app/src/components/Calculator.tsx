  ...
  return (
    <div className="space-y-6">
      ...
      <Collapsible title="Berechnungsergebnis" defaultOpen={true} badge="✓">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              <p className="text-sm font-semibold text-slate-900">
                💼 Monatlicher Bruttolohn: CHF {formatCurrency(grossSalary, 'CHF').replace('CHF ', '')}
              </p>
              <p className="text-xs text-slate-600">Jahresbrutto: CHF {formatCurrency(grossSalary * salaryMonths, 'CHF').replace('CHF ', '')} ({salaryMonths}× Gehalt)</p>
            </div>
            <div className="text-right">
              <p className="text-lg font-bold text-blue-600">EUR {formatCurrency(grossSalary * exchangeRate, 'EUR').replace('EUR ', '')}</p>
              <p className="text-xs text-slate-500">Wechselkurs: {exchangeRate.toFixed(4)}</p>
            </div>
          </div>
        </div>
        ...
        {/* Erklärung nur bei 13/14 Gehältern zeigen */}
        {salaryMonths > 12 && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
            <p className="text-sm font-semibold text-green-900 mb-1">
              💰 {salaryMonths} Gehälter: Steuerlicher Vorteil
            </p>
            <p className="text-xs text-green-800">
              {salaryMonths === 13 && 'Das 13. Gehalt wird in Österreich mit nur 6% besteuert (statt progressiv bis 50%).'}
              {salaryMonths === 14 && 'Das 13. und 14. Gehalt werden in Österreich mit je 6% besteuert (statt progressiv bis 50%).'}
              {' '}Für Vergleichbarkeit: Jahresnetto ({formatCurrency(result.yearlyNetEUR, 'EUR')}) 
              ÷ 12 Monate = <strong>{formatCurrency(result.averageMonthlyNetEUR, 'EUR')}/Monat Ø</strong>.
            </p>
          </div>
        )}
        ...
        <div className="bg-white rounded-lg p-4 mb-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-3">
            <span className="text-sm font-semibold text-slate-700">📊 Effektive Gesamtbelastung</span>
            <span className="text-2xl font-bold text-blue-900">{result.effectiveTaxRate.toFixed(1)}%</span>
          </div>
          <div className="w-full bg-slate-200 rounded-full h-4">
            ...
            >
              <span className="text-[10px] text-white font-semibold">{result.effectiveTaxRate.toFixed(1)}%</span>
            </div>
          </div>
          <p className="text-xs text-slate-500 mt-2">Inkl. CH-Abzüge, Quellensteuer und AT-Steuer (nach DBA-Anrechnung)</p>
        </div>
